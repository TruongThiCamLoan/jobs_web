...
// ✅ Import DonateButton dưới dạng dynamic, tắt SSR
const DonateButton = dynamic(() => import("@/components/DonateButton"), {
  ssr: false,
});

// ===== Dữ liệu mẫu (giữ nguyên để test)
  ...
  <nav className="bg-gray-800 p-4 shadow-lg sticky top-0 z-10 border-b border-blue-800">
    <div className="max-w-7xl mx-auto flex justify-between items-center">
      ...
      {!account ? (
        ...
        <div className="bg-gray-700 px-3 py-2 rounded-full text-sm border border-blue-500">
    ...
    <ul className="space-y-2">
      {STATIC_DONORS.map((d) => (
        <li key={d.id} className="flex justify-between text-sm text-gray-300">
          <span>{d.address}</span>
          <span className="text-green-300">{d.amount}</span>
        </li>
      ))}
    </ul>
  </section>
...
export default function Home() {
  ...
  // ---- Kiểm tra ví khi load lại trang ----
  useEffect(() => {
    if (typeof window !== "undefined" && window.ethereum) {
      window.ethereum
